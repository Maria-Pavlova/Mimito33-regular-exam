...
suite("Add Books page", function () {
  test("Page title", async function () {
    ...
    assert.ok(body.includes("<title>Add Book</title>"));
  });
  ...
  test("Book HTML form", async function () {
    ...
    let authorFieldFound = body.includes(
  ...
  test("Add invalid book", async function () {
    ...
    let errMsg = body.includes(
      "Cannot add book. Name and author fields are required!"
    );
    ...
    body = await res.text();
    assert.ok(
      body.includes("Added books: <b>3</b>"),
